<?php


namespace App\Core\Utils;


use Tracy\Dumper;
use Tracy\IBarPanel;

class TracyDBBar implements IBarPanel
{

    /**
     * Base64 icon for Tracy panel.
     * @var string
     */
    public $icon = 'data:image/svg+xml;utf8;base64,';

    /**
     * Title
     * @var string
     */
    public $title = 'PDO logger';

    /**
     * Renders HTML code for custom tab.
     * @return string
     */
    function getTab()
    {
        $html = '<img src="'.$this->icon.'" alt="PDO queries logger" /> ';
        if(isset($_SESSION['db_log'])) {
            $totalTime = array_sum(array_column($_SESSION['db_log'], 'time'));
            $html = '<img src="'.$this->icon.'" alt="PDO queries logger" /> '.$totalTime.' ms';
        }
        return $html;
    }

    private function parse(array $logs)
    {
        //return Dumper::toHtml($log);
        $result = [];
        foreach ($logs as $log)
        {
            $row = '
            <tr>
                <td>'.$log['query'].'</td>
                <td>'.$log['time'].' ms</td>
            </tr>';
            if(!empty($log['args']))
            {
                $row .='<tr><td colspan="2"> parameters:'.Dumper::toHtml($log['args'], [Dumper::COLLAPSE => true]).'</td></tr>';
            }
            $result[] = $row;
        }
        return $result;
    }

    /**
     * Renders HTML code for custom panel.
     * @return string
     */
    function getPanel()
    {
        $content = 'no queries';
        if(isset($_SESSION['db_log'])) {
            $content = $this->parse($_SESSION['db_log']);
            $content = implode($content);
        }
        unset($_SESSION['db_log']);
        $html =  '
        <div style="max-width: 600px; max-height: 600px">
            <h1>
                <div style="width: 20px; height: 20px; display: inline-block;"><img src="'.$this->icon.'"/></div> 
                 PDO queries
            </h1>
            <div class="tracy-inner">
                <table>
                   '.$content.'
                </table>    
            </div>
        </div>';
        return $html;
    }
}